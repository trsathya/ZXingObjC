 ...
 */
- (ZXBitMatrix *)renderResult:(ZXBoolArray *)code width:(int)width height:(int)height sidesMargin:(int)sidesMargin verticalPadding:(int)verticalPadding {
  ...
  for (int inputX = 0, outputX = leftPadding; inputX < inputWidth; inputX++, outputX += multiple) {
    if (code.array[inputX]) {
      [output setRegionAtLeft:outputX top:verticalPadding width:multiple height:outputHeight];
    }
  }
  return output;
}
